    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        ...
        <input type="text" name="fullName" value={form.fullName} onChange={handleChange} required placeholder="Ej. Juan Flores"
          className="w-full border border-gray-200 px-4 py-3 text-sm font-body text-gray-900 placeholder-gray-400 focus:outline-none focus:border-brand-500 transition-colors" />
      </div>
      ...
      <div>
        ...
        <div className="flex gap-3">
          {(['phone', 'email'] as const).map((method) => (
            <label key={method} className={`flex-1 flex items-center justify-center gap-2 py-2.5 border cursor-pointer text-sm font-body transition-colors ${
              form.contactMethod === method ? 'border-brand-600 bg-brand-50 text-brand-700' : 'border-gray-200 text-gray-500 hover:border-gray-300'
            }`}>
      ...
      <button type="submit" disabled={loading}
        className="w-full bg-brand-600 text-white py-4 text-sm font-body font-medium tracking-widest uppercase flex items-center justify-center gap-2 hover:bg-brand-700 transition-colors disabled:opacity-70 active:scale-[0.99]">
